...
class Manager {
    ...
    public function __construct($gcal = null)
    {
        if (!$gcal) {
            ...
            $params = array(
                'email'    => \Config::get('mustached.calendar.google_calendar_email'),
                'password' => \Config::get('mustached.calendar.google_calendar_password'),
            );
    ...
    public function get_next_events($number = 20)
    {
        if (!$this->gcal->authenticate()) {
            ...
            $evts = $this->gcal->getEvents(\Config::get('mustached.calendar.google_calendar_id'), $number, date('Y-m-d'));
        }
